...
def menu():
    if request.method == "POST":
        if not request.form.get("meal"):
            ...
            return apology("Write a positive integer of amount.")
        amount = int(request.form.get("amount"))
        if amount < 0:
            return apology("Invalid amount")
        if not amount:
            return apology("Too few amount.")
        meal = request.form.get("meal")
        print(meal);
#    SELECT meal and price from menu
        rows = db.execute("SELECT meal, price FROM menu WHERE meal = :meal", meal=request.form.get("meal"))
        print(rows)
        for row in rows:
            meals=row["meal"]
            price=row["price"]
        ...
        # meal = request.form.get("meal")
        # cost = price * amount
        return render_template("menud.html", cost=cost)
...
#             price=row["price"]
#         # meal =  request.form.get("meal")
#         return render_template("menud.html", meals=meals)
#     else:
...
def history():
    ...
    rows = db.execute("SELECT meal, amount, cost, ordered FROM history WHERE user_id=:user_id ORDER BY ordered ASC", user_id = session["user_id"])
